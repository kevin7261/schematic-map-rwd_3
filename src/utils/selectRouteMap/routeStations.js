/**
 * 🗺️ 選擇路線圖（select_route_map）— 站點推導與路線顏色工具
 *
 * ⚠️ 本檔為「選擇路線圖」圖層**獨立複製**之版本，刻意不與 leaflet_josm_draw 的
 *    src/utils/leafletDrawStations.js 共用任何程式，避免互相牽動。
 *
 * 由載入的城市路線（折線）與站點黑點，推導三類站點：
...
export const routeMapColorForIndex = (i) => {
  const idx = Number.isFinite(i) && i >= 0 ? Math.floor(i) : 0;
  if (idx < ROUTE_MAP_PALETTE.length) return ROUTE_MAP_PALETTE[idx].hex;
  const hue = (idx * 137.508) % 360;
  return `hsl(${hue.toFixed(1)}, 70%, 45%)`;
};

/** 依路線索引取顏色名稱（路線以顏色為名） */
export const routeMapColorNameForIndex = (i) => {
  const idx = Number.isFinite(i) && i >= 0 ? Math.floor(i) : 0;
  if (idx < ROUTE_MAP_PALETTE.length) return ROUTE_MAP_PALETTE[idx].name;
  return `顏色 ${idx + 1}`;
};
 ...
 */
export const computeRouteMapRouteStations = (lines, blackDots) => {
  const safeLines = Array.isArray(lines)
    ? lines.filter((l) => l && Array.isArray(l.latlngs) && l.latlngs.length >= 2)
    : [];
  const { connects, blacks } = computeRouteMapStations(safeLines, blackDots);
