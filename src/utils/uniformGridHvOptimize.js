...
export function buildHvOptimizePayload(routes, gridX, gridY) {
  ...
  return {
    ...
    task: {
      ...
      topologyRule:
        '只可平移紅／藍／粉紅點的 (x,y)；拓撲結構關係不可變：edges 連接、route id 順序、轉乘 id 匯流組合、端點歸屬、id/kind 集合皆同 payload；不可新增/刪除頂點；相鄰拓撲點之間僅一條直線；黑站不在 network 內；HV 拉直不製造折角；偏水平→水平 HV、偏垂直→垂直 HV、45°→兩者皆可，不可翻向',
    },
...
5. endpoint id 仍為同一路線（段）的起點或終點，不可變成中途匯流角色。
6. 回傳 coords 必須涵蓋所有 movablePoints 的 id；未移動的點請回傳原座標。
...
export function buildHvOptimizeUserPrompt(payload, roundIndex) {
  ...
  return [
    ...
    '【重要】topology 固定（拓撲結構關係不可變：誰連誰、route 順序、轉乘匯流）；只能平移紅/藍/粉紅座標以拉直路線（HV 直線），不可製造新路線轉折；偏水平段→水平 HV、偏垂直段→垂直 HV、45°→兩者皆可，不可翻向。',
