/**
 * 資料處理管線（taipei_*_dp）：執行步驟與 executeTaipeiTest3Flow 逐行對應，但僅依賴 @/utils/taipeiDataProcTest3（與測試_3 之 taipeiTest3 目錄分離）。
 * g3→h3 匯出列來源為 taipei_b3_dp，非 taipei_a3。
 */
...
export async function executeTaipeiDataProcTest3_A3_To_B3() {
  const dataStore = useDataStore();
  const a3 = dataStore.findLayerById('taipei_a3');
  const b3 = dataStore.findLayerById('taipei_b3_dp');
  if (!b3) {
    console.warn('executeTaipeiDataProcTest3_A3_To_B3：找不到 taipei_b3');
    return;
  }
  ...
  if (!b3.visible) {
    b3.visible = true;
    dataStore.saveLayerState('taipei_b3_dp', { visible: true });
  }
}

/** b3：交叉點直線化 → 寫入 c3 */
export function executeTaipeiDataProcTest3_B3_To_C3() {
  const dataStore = useDataStore();
  const b3 = dataStore.findLayerById('taipei_b3_dp');
  const c3 = dataStore.findLayerById('taipei_c3_dp');
  if (!b3?.spaceNetworkGridJsonData?.length || !c3) {
    console.warn('executeTaipeiDataProcTest3_B3_To_C3：缺少 b3 路網或 c3 圖層');
    return;
  }
  ...
  if (!c3.visible) {
    c3.visible = true;
    dataStore.saveLayerState('taipei_c3_dp', { visible: true });
  }
}

/** c3：座標正規化 → 寫入 d3 */
export function executeTaipeiDataProcTest3_C3_To_D3() {
  const dataStore = useDataStore();
  const c3 = dataStore.findLayerById('taipei_c3_dp');
  const d3Layer = dataStore.findLayerById('taipei_d3_dp');
  if (!c3?.spaceNetworkGridJsonData?.length || !d3Layer) {
    console.warn('executeTaipeiDataProcTest3_C3_To_D3：缺少 c3 路網或 d3 圖層');
    return;
  }
  ...
  d3Layer.dashboardData = {
    ...
    sourceLayerId: 'taipei_c3_dp',
    coordNormalize: true,
    ...out.meta,
  };
  if (!d3Layer.visible) {
    d3Layer.visible = true;
    dataStore.saveLayerState('taipei_d3_dp', { visible: true });
  }
}

/** d3：刪除無紅／藍（connect）之整欄／整列並壓縮座標 → 寫入 e3 */
export function executeTaipeiDataProcTest3_D3_To_E3() {
  const dataStore = useDataStore();
  const d3Layer = dataStore.findLayerById('taipei_d3_dp');
  const e3Layer = dataStore.findLayerById('taipei_e3_dp');
  if (!d3Layer?.spaceNetworkGridJsonData?.length || !e3Layer) {
    console.warn('executeTaipeiDataProcTest3_D3_To_E3：缺少 d3 路網或 e3 圖層');
    return;
  }
  ...
  if (!e3Layer.visible) {
    ...
    dataStore.saveLayerState('taipei_e3_dp', { visible: true });
  }
}

/** e3：對角線轉水平垂直（applyHVToE3Network）→ 寫入 f3 */
export function executeTaipeiDataProcTest3_E3_To_F3() {
  const dataStore = useDataStore();
  const e3Layer = dataStore.findLayerById('taipei_e3_dp');
  const f3Layer = dataStore.findLayerById('taipei_f3_dp');
  if (!e3Layer?.spaceNetworkGridJsonData?.length || !f3Layer) {
    console.warn('executeTaipeiDataProcTest3_E3_To_F3：缺少 e3 路網或 f3 圖層');
    return;
  }
  ...
  if (!f3Layer.visible) {
    ...
    dataStore.saveLayerState('taipei_f3_dp', { visible: true });
  }
}
 ...
 */
export function executeTaipeiDataProcTest3_F3_To_G3() {
  const dataStore = useDataStore();
  const f3Layer = dataStore.findLayerById('taipei_f3_dp');
  const g3Layer = dataStore.findLayerById('taipei_g3_dp');
  if (!f3Layer?.spaceNetworkGridJsonData?.length || !g3Layer) {
    console.warn('executeTaipeiDataProcTest3_F3_To_G3：缺少 f3 路網或 g3 圖層');
    return;
  }
  ...
  if (!g3Layer.visible) {
    ...
    dataStore.saveLayerState('taipei_g3_dp', { visible: true });
  }
...
/** g3：將 taipei_b3_dp 路段匯出列之中段站（黑點）依路線／路段序對應到 g3 折線，弧長均分 → 寫入 h3 */
export function executeTaipeiDataProcTest3_G3_To_H3() {
  const dataStore = useDataStore();
  const b3DpRowSource = dataStore.findLayerById('taipei_b3_dp');
  const g3Layer = dataStore.findLayerById('taipei_g3_dp');
  const h3Layer = dataStore.findLayerById('taipei_h3_dp');
  if (!g3Layer?.spaceNetworkGridJsonData?.length || !h3Layer) {
    console.warn('executeTaipeiDataProcTest3_G3_To_H3：缺少 g3 路網或 taipei_h3 圖層');
    return;
  }

  const check = assertA3RowsForG3ToH3(b3DpRowSource);
  if (!check.ok) {
    console.warn(`executeTaipeiDataProcTest3_G3_To_H3：${check.message}`);
    return;
  }
  ...
  h3Layer.dashboardData = {
    ...
    sourceLayerId: 'taipei_g3_dp',
    a3SourceLayerId: 'taipei_b3_dp',
  ...
  if (!h3Layer.visible) {
    ...
    dataStore.saveLayerState('taipei_h3_dp', { visible: true });
  }
}

/** h3：僅在既有頂點依黑點切段（不改座標、不重算紅黑站別、無線上權重數字）→ 寫入 taipei_i3 */
export function executeTaipeiDataProcTest3_H3_To_I3() {
  const dataStore = useDataStore();
  const h3Layer = dataStore.findLayerById('taipei_h3_dp');
  const i3Layer = dataStore.findLayerById('taipei_i3_dp');
  if (!h3Layer?.spaceNetworkGridJsonData?.length || !i3Layer) {
    console.warn('executeTaipeiDataProcTest3_H3_To_I3：缺少 h3 路網或 taipei_i3 圖層');
    return;
  }
  ...
  i3Layer.layerInfoData = {
    splitSourceLayerId: 'taipei_h3_dp',
  ...
  i3Layer.dashboardData = {
    ...
    sourceLayerId: 'taipei_h3_dp',
    segmentsBeforeSplit: priorCount,
  };
  if (!i3Layer.visible) {
    i3Layer.visible = true;
    dataStore.saveLayerState('taipei_i3_dp', { visible: true });
  }
}

/** i3：讀 mrt_link_volume CSV → 對應切段 station_weights → 寫入 taipei_j3（含 dataTableData） */
export async function executeTaipeiDataProcTest3_I3_To_J3() {
  const dataStore = useDataStore();
  const i3Layer = dataStore.findLayerById('taipei_i3_dp');
  const j3Layer = dataStore.findLayerById('taipei_j3_dp');
  if (!i3Layer?.spaceNetworkGridJsonData?.length || !j3Layer) {
    console.warn('executeTaipeiDataProcTest3_I3_To_J3：缺少 i3 路網或 taipei_j3 圖層');
    return;
  }
  ...
  if (!trafficData?.rows?.length) {
    console.warn('executeTaipeiDataProcTest3_I3_To_J3：CSV 無資料或讀取失敗');
  }
  ...
  if (!j3Layer.visible) {
    ...
    dataStore.saveLayerState('taipei_j3_dp', { visible: true });
  }
}
 ...
 */
export function executeTaipeiDataProcTest3_K3_To_L3() {
  const dataStore = useDataStore();
  const k3Layer = dataStore.findLayerById('taipei_k3_dp');
  const l3Layer = dataStore.findLayerById('taipei_l3_dp');
  if (!k3Layer || !l3Layer) {
    console.warn('executeTaipeiDataProcTest3_K3_To_L3：缺少 taipei_k3 或 taipei_l3 圖層');
    return;
  }
  const k3Routes = k3Layer.spaceNetworkGridJsonDataK3Tab;
  if (!Array.isArray(k3Routes) || k3Routes.length === 0) {
    console.warn('executeTaipeiDataProcTest3_K3_To_L3：taipei_k3 尚無 K3 分頁路網（請先載入 k3 之 JSON）');
    return;
  }
  ...
  l3Layer.layerInfoData =
    baseInfo && typeof baseInfo === 'object'
      ? { ...baseInfo, copiedFromLayerId: 'taipei_k3_dp' }
      : { copiedFromLayerId: 'taipei_k3_dp' };
  ...
  if (!l3Layer.visible) {
    ...
    dataStore.saveLayerState('taipei_l3_dp', { visible: true });
  }

  dataStore.saveLayerState('taipei_l3_dp', {
 ...
 */
export async function executeTaipeiDataProcTest3_L3_To_M3() {
  const dataStore = useDataStore();
  const l3Layer = dataStore.findLayerById('taipei_l3_dp');
  const m3Layer = dataStore.findLayerById('taipei_m3_dp');
  if (!l3Layer || !m3Layer) {
    console.warn('executeTaipeiDataProcTest3_L3_To_M3：缺少 taipei_l3 或 taipei_m3 圖層');
    return;
  }
  const l3Routes = l3Layer.spaceNetworkGridJsonDataL3Tab;
  if (!Array.isArray(l3Routes) || l3Routes.length === 0) {
    console.warn(
      'executeTaipeiDataProcTest3_L3_To_M3：taipei_l3 尚無 L3 分頁路網（請先以 k3→l3 複製或載入 l3 之 JSON）'
    );
    return;
  }
  ...
  m3Layer.layerInfoData =
    baseInfo && typeof baseInfo === 'object'
      ? { ...baseInfo, copiedFromLayerId: 'taipei_l3_dp' }
      : { copiedFromLayerId: 'taipei_l3_dp' };
  ...
  if (!m3Layer.visible) {
    ...
    dataStore.saveLayerState('taipei_m3_dp', { visible: true });
  }

  dataStore.saveLayerState('taipei_m3_dp', {
