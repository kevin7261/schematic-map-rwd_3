 ...
 * Cursor Agent 依 skill 回寫 hv_response.json + hv_audit.json（LLM audit）；
 * App 檢查 audit pass 後直接載入座標預覽。不用 API Key、不用剪貼簿。
 *
...
import { mergeHvNetworkKind, routesUseStoredMarkerKinds } from './routeAdjustAiTestRouteMarkers.js';
...
App 不做幾何驗證；反驗證（越界、交叉、方向翻轉、360° 環序等）由 LLM audit（validate-ai-test-hv skill）在對話中推理完成，寫入 hv_audit.json。
