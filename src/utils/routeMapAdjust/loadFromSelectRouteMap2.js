/**
 * 🗺️ 路線圖轉換骨架2（route_map_adjust_2）— 「從選擇路線圖載入」邏輯（composable）
 *
 * ⚠️ 本檔為「路線圖調整」圖層**獨立複製**之版本（複製自 loadFromSelectRouteMap.js），
 *    刻意不與 select_route_map 的程式共用；
...
export const ROUTE_MAP_ADJUST_2_LAYER_ID = 'route_map_adjust_2';
/** 來源圖層 id：「選擇路線圖」 */
 ...
 */
export function useRouteMapAdjust2(dataStore) {
  const adjustLayer = computed(() => dataStore.findLayerById(ROUTE_MAP_ADJUST_2_LAYER_ID));
