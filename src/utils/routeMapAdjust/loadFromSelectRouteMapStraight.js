/**
 * 🗺️ 路線圖調整直線骨架（route_map_adjust_straight）— 「從選擇路線圖載入」邏輯（composable）
 *
 * ⚠️ 本檔為「路線圖轉換直線骨架」圖層**獨立複製**之版本，刻意不與 route_map_adjust 的程式共用；
...
import { buildRouteMapAdjustStraightSkeleton } from './straightenLinesAtRedBlue.js';

/** 本圖層 id（與 dataStore 內定義一致） */
export const ROUTE_MAP_ADJUST_STRAIGHT_LAYER_ID = 'route_map_adjust_straight';
/** 來源圖層 id：「選擇路線圖」 */
...
/**
 * 「路線圖轉換直線骨架」composable。
 * @param {*} dataStore Pinia dataStore 實例
 */
export function useRouteMapAdjustStraight(dataStore) {
  const adjustLayer = computed(() => dataStore.findLayerById(ROUTE_MAP_ADJUST_STRAIGHT_LAYER_ID));
  ...
  const loadFromSelectRouteMap = () => {
    ...
    try {
      ...
      dst.routeMapAdjustStraightenedLines = null;
      dst.routeMapAdjustStraightenedBlackDots = null;
      dst.routeMapAdjustStraightenedStationMeta = null; // 骨架由按鈕觸發
      dst.routeMapAdjustSource = src.selectRouteMapSource
        ? `從選擇路線圖載入：${src.selectRouteMapSource}`
        : '從選擇路線圖載入';
      dataStore.requestRouteMapAdjustStraightFit();
    } finally {
  ...
  /** 🧹 清除目前調整中的路線 */
  const clearRouteMapAdjustStraight = () => {
    const dst = adjustLayer.value;
    if (dst) {
      ...
      dst.routeMapAdjustStraightenedLines = null;
      dst.routeMapAdjustStraightenedBlackDots = null;
      dst.routeMapAdjustStraightenedStationMeta = null;
    }
  };
  ...
  /** 🦴 切換骨架：紅/藍錨點間先拉直，再變成骨架；再按一次還原 */
  const toggleSkeleton = () => {
    const lyr = adjustLayer.value;
    if (!lyr) return;
    if (lyr.routeMapAdjustSkeleton) {
      lyr.routeMapAdjustSkeleton = null;
      lyr.routeMapAdjustStraightenedLines = null;
      lyr.routeMapAdjustStraightenedBlackDots = null;
      lyr.routeMapAdjustStraightenedStationMeta = null;
      return;
    }
    ...
    const stationMeta = lyr.routeMapAdjustStationMeta || null;
    const { skeleton, straightenedLines, straightenedBlackDots, straightenedStationMeta } =
      buildRouteMapAdjustStraightSkeleton(lines, blackDots, stationMeta);
    lyr.routeMapAdjustStraightenedLines = straightenedLines;
    lyr.routeMapAdjustStraightenedBlackDots = straightenedBlackDots;
    lyr.routeMapAdjustStraightenedStationMeta = straightenedStationMeta;
    lyr.routeMapAdjustSkeleton = skeleton;
    dataStore.requestRouteMapAdjustStraightFit();
  };
