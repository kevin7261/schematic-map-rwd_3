<script setup>
  ...
  } from '@/utils/routeAdjustAiTestRouteMarkers.js';
  import { countUniformGridBlackStationsOnSegment } from '@/utils/dataProcessor.js';
  import { fingerprintUniformGridRoutes } from '@/utils/routeAdjustAiTestHvOptimize.js';
  ...
  const getHvPreviewDrawKey = () => {
    ...
    if (layer?.layerId !== 'route_adjust_ai_test_layer' || !layer.hvOptimizeLastResult) return '';
  ...
  const shouldSkipGridRedraw = (width, height, margin) => {
    const existingSvg = d3.select('#route-adjust-grid-scaling-container').select('svg');
  ...
  const getUniformGridTooltip = () => {
    if (!uniformGridTooltip) {
      d3.select('body').selectAll('.route-adjust-grid-scaling-uniform-grid-tooltip').remove();
      uniformGridTooltip = d3
        .select('body')
        .append('div')
        .attr('class', 'route-adjust-grid-scaling-uniform-grid-tooltip')
  ...
  const setActiveLayerTab = (layerId) => {
    ...
    // 立即清除 SVG 內容，避免重疊
    d3.select('#route-adjust-grid-scaling-container').selectAll('svg').remove();
  ...
  const loadLayerData = async (layerId) => {
    try {
      ...
      if (targetLayer.drawJsonData && targetLayer.drawJsonData.type === 'grid') {
        ...
        if (
          targetLayer.layerId === 'route_adjust_ai_test_layer' &&
   ...
   */
  const getDimensions = () => {
    const container = document.getElementById('route-adjust-grid-scaling-container');
  ...
  const drawGridSchematic = () => {
    ...
    // 創建 SVG 元素
    const svg = d3
      .select('#route-adjust-grid-scaling-container')
      .append('svg')
  ...
  const drawUniformGridLines = (svg, width, height, margin) => {
    ...
    if (Array.isArray(routes) && routes.length > 0) {
      ...
      if (
        previewLayer?.layerId === 'route_adjust_ai_test_layer' &&
  ...
  */
  ...
  watch(
    () => {
      if (!activeLayerTab.value) return null;
      const layer = visibleLayers.value.find((l) => l.layerId === activeLayerTab.value);
      return layer?.layerId === 'route_adjust_ai_test_layer' ? layer.hvOptimizeLastResult : null;
    },
    async () => {
      if (activeLayerTab.value) {
        await nextTick();
      ...
      <div v-if="currentLayerSummary" class="flex-grow-1 d-flex flex-column" style="min-height: 0">
        ...
        <div class="flex-grow-1 d-flex flex-column" style="min-height: 0">
          <div class="flex-grow-1" style="min-height: 0">
            ...
            <div
              id="route-adjust-grid-scaling-container"
              class="w-100 h-100"
...
<style scoped>
  ...
  #route-adjust-grid-scaling-container {
    position: relative;
    overflow: hidden;
  }
