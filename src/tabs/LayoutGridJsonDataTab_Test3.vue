<script setup>
  /**
   * 📄 LayoutGridJsonDataTab_Test3.vue - 佈局網格 JSON 數據測試3分頁組件 (Layout Grid JSON Data Test3 Tab Component)
   *
   * 這是一個專門用於顯示圖層佈局網格 JSON 測試3數據的組件，提供多圖層的 JSON 數據查看功能。
   * 該組件能夠動態顯示當前開啟圖層的佈局網格 JSON 測試3數據，幫助用戶查看和檢查數據結構。
   *
   * 🎯 主要功能 (Core Features):
   * 1. 📄 佈局網格 JSON 測試3數據顯示：顯示圖層的佈局網格 JSON 測試3數據 (layoutGridJsonData_Test3)
   *    - 格式化 JSON 字符串顯示
   *    - 支持多圖層切換查看
   *    - 自動檢測數據可用性
   * 2. 🔄 動態圖層切換：支援多圖層的數據展示
   ...
   * - 研究工具的測試3數據檢查
   *
   * @component LayoutGridJsonDataTab_Test3
   * @version 1.0.0
   * @author Kevin Cheng
   * @since 1.0.0
   */
  ...
  const visibleLayers = computed(() => {
    ...
    return allLayers.filter((layer) => layer.visible && layer.layoutGridJsonData_Test3);
  });
  ...
  /**
   * 📊 當前圖層的佈局網格 JSON 測試2數據 (Current Layer Layout Grid JSON Test3 Data)
   */
  const currentLayerJsonData = computed(() => {
    if (!activeLayerTab.value) return null;
    const layer = visibleLayers.value.find((l) => l.layerId === activeLayerTab.value);
    return layer ? layer.layoutGridJsonData_Test3 || null : null;
  });
  ...
  <div class="d-flex flex-column my-bgcolor-gray-200 h-100">
    ...
    <div v-if="visibleLayers.length > 0" class="flex-grow-1 overflow-auto my-bgcolor-white">
      ...
      <div class="p-3 border-bottom">
        ...
        <div class="my-title-xs-gray">佈局網格 JSON 測試3數據 (layoutGridJsonData_Test3)</div>
      </div>
