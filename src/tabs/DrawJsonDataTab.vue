<script setup>
  /**
   * 📄 DrawJsonDataTab.vue - 繪製數據分頁組件 (Draw JSON Data Tab Component)
   *
   * 這是一個專門用於顯示圖層繪製 JSON 數據的組件，提供多圖層的繪製數據查看功能。
   * 該組件能夠動態顯示當前開啟圖層的繪製 JSON 數據，幫助用戶查看和檢查繪製數據結構。
   *
   * 🎯 主要功能 (Core Features):
   * 1. 📄 繪製 JSON 數據顯示：顯示圖層的繪製 JSON 數據 (drawJsonData)
   *    - 格式化 JSON 字符串顯示
   *    - 支持多圖層切換查看
   *    - 自動檢測數據可用性
   * 2. 🔄 動態圖層切換：支援多圖層的數據展示
  ...
  const visibleLayers = computed(() => {
    ...
    return allLayers.filter((layer) => layer.visible && layer.drawJsonData);
  });
  ...
  /**
   * 📊 當前圖層的繪製 JSON 數據 (Current Layer Draw JSON Data)
   */
  const currentLayerDrawJsonData = computed(() => {
  ...
  const formattedJsonString = computed(() => {
    if (!currentLayerDrawJsonData.value) return null;
    try {
      return JSON.stringify(currentLayerDrawJsonData.value, null, 2);
    } catch (error) {
      console.error('格式化 JSON 數據時發生錯誤:', error);
      return String(currentLayerDrawJsonData.value);
    }
  });
  ...
  <div class="d-flex flex-column my-bgcolor-gray-200 h-100">
    ...
    <div v-if="visibleLayers.length > 0" class="flex-grow-1 overflow-auto my-bgcolor-white">
      ...
      <div class="p-3 border-bottom">
        ...
        <div class="my-title-xs-gray">繪製 JSON 數據 (drawJsonData)</div>
      </div>
      ...
      <div v-else-if="currentLayerDrawJsonData === null" class="p-3 text-center">
        <div class="my-title-md-gray py-5">此圖層沒有可用的繪製 JSON 數據</div>
      </div>
    </div>
    ...
    <div v-else class="flex-grow-1 d-flex align-items-center justify-content-center">
      <div class="text-center">
        <div class="my-title-md-gray p-3">沒有開啟的圖層或沒有繪製 JSON 數據</div>
      </div>
    </div>
  </div>
</template>
