<script setup>
  /**
   * 📄 JsonDataTab.vue - 原始 JSON 數據分頁組件 (Original JSON Data Tab Component)
   *
   * 這是一個專門用於顯示圖層原始 JSON 數據的組件，提供多圖層的 JSON 數據查看功能。
   * 該組件能夠動態顯示當前開啟圖層的原始 JSON 數據，幫助用戶查看和檢查數據結構。
   *
   * 🎯 主要功能 (Core Features):
   * 1. 📄 原始 JSON 數據顯示：顯示圖層的原始 JSON 數據 (jsonData)
   *    - 格式化 JSON 字符串顯示
   *    - 支持多圖層切換查看
   *    - 自動檢測數據可用性
   * 2. 🔄 動態圖層切換：支援多圖層的數據展示
  ...
  const visibleLayers = computed(() => {
    ...
    // 空間網絡網格流程會把 execute_* 的結果寫到 spaceNetworkGridJsonData
    return allLayers.filter(
      (layer) => layer.visible && (layer.spaceNetworkGridJsonData || layer.jsonData)
    );
  });
  ...
  /**
   * 📊 當前圖層的原始 JSON 數據 (Current Layer Original JSON Data)
   */
  const currentLayerJsonData = computed(() => {
    if (!activeLayerTab.value) return null;
    const layer = visibleLayers.value.find((l) => l.layerId === activeLayerTab.value);
    // 優先顯示 spaceNetworkGridJsonData；沒有才回退 jsonData
    return layer ? layer.spaceNetworkGridJsonData || layer.jsonData || null : null;
  });
  ...
  <div class="d-flex flex-column my-bgcolor-gray-200 h-100">
    ...
    <div v-if="visibleLayers.length > 0" class="flex-grow-1 overflow-auto my-bgcolor-white">
      ...
      <div class="p-3 border-bottom">
        ...
        <div class="my-title-xs-gray">原始 JSON 數據 (jsonData)</div>
      </div>
      ...
      <div v-else-if="currentLayerJsonData === null" class="p-3 text-center">
        <div class="my-title-md-gray py-5">此圖層沒有可用的原始 JSON 數據</div>
      </div>
    </div>
    ...
    <div v-else class="flex-grow-1 d-flex align-items-center justify-content-center">
      <div class="text-center">
        <div class="my-title-md-gray p-3">沒有開啟的圖層或沒有原始 JSON 數據</div>
      </div>
    </div>
  </div>
</template>
