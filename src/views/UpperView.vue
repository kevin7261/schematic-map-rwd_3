...
<script>
  ...
  import RouteAdjustGridScalingTab from '../tabs/RouteAdjustGridScalingTab.vue';
  ...
  import SelectRouteMapGeojsonTab from '../tabs/SelectRouteMapGeojsonTab.vue';
  import RouteMapAdjustTab from '../tabs/RouteMapAdjustTab.vue';
  import RouteMapAdjustTab2 from '../tabs/RouteMapAdjustTab2.vue';
  import RouteMapAdjustStraightTab from '../tabs/RouteMapAdjustStraightTab.vue';
  import RouteSchematicTab from '../tabs/RouteSchematicTab.vue';
  ...
  export default {
    ...
    components: {
      ...
      MapTab,
      SelectRouteMapTab,
      SelectRouteMapGeojsonTab,
      RouteMapAdjustTab,
      RouteMapAdjustTab2,
      RouteMapAdjustStraightTab,
      RouteSchematicTab,
    },
    ...
    setup(props, { emit }) {
      ...
      /** 📊 網格縮放（路線調整）視圖組件引用 */
      const RouteAdjustGridScalingTab = ref(null);
      /** 📊 空間網絡網格視圖組件引用 */
      ...
      /** 📊 網格縮放（路線調整）容器引用 (用於控制滑鼠事件) */
      const routeAdjustGridScalingContainerRef = ref(null);
      /** 📊 空間網絡網格容器引用 */
      ...
      /** 🗺️ 路線圖調整（route_map_adjust）顯示元件引用 */
      const RouteMapAdjustTab = ref(null);
      const routeMapAdjustContainerRef = ref(null);
      /** 🗺️ 路線圖轉換骨架2（route_map_adjust_2）顯示元件引用 */
      const RouteMapAdjustTab2 = ref(null);
      const routeMapAdjust2ContainerRef = ref(null);
      /** 🗺️ 路線圖轉換直線骨架（route_map_adjust_straight）顯示元件引用 */
      const RouteMapAdjustStraightTab = ref(null);
      const routeMapAdjustStraightContainerRef = ref(null);
      /** 🗺️ 示意圖佈局（schematic_rma_*）獨立顯示元件引用 */
      const RouteSchematicTab = ref(null);
      const routeSchematicContainerRef = ref(null);
      ...
      const allPossibleTabs = [
        'd3js',
        'grid-scaling',
        'route-adjust-grid-scaling',
        'select-route-map',
        'select-route-map-geojson',
        'route-map-adjust',
        'route-map-adjust-2',
        'route-map-adjust-straight',
        'route-schematic',
        'space-network-grid',
        'dashboard',
        'processed-json-data',
      ...
      const handleActiveLayerChange = (layerId) => {
        ...
        dataStore.setControlActiveLayerId(layerId ?? null);
      };
      ...
      watch(
        ...
        ([dragging, tab]) => {
          nextTick(() => {
            ...
            // 處理網格縮放（路線調整）容器
            if (routeAdjustGridScalingContainerRef.value) {
              if (dragging && tab === 'route-adjust-grid-scaling') {
                routeAdjustGridScalingContainerRef.value.style.pointerEvents = 'none';
              } else {
                routeAdjustGridScalingContainerRef.value.style.pointerEvents = 'auto';
              }
            }
      ...
      watch(
        ...
        (newTab) => {
          ...
          } else if (newTab === 'route-adjust-grid-scaling') {
            nextTick(() => {
              setTimeout(() => {
                if (RouteAdjustGridScalingTab.value && RouteAdjustGridScalingTab.value.resize) {
                  RouteAdjustGridScalingTab.value.resize();
                }
              }, 100);
            });
          ...
          } else if (newTab === 'route-map-adjust') {
            nextTick(() => {
              setTimeout(() => {
                if (RouteMapAdjustTab.value && RouteMapAdjustTab.value.resize) {
                  RouteMapAdjustTab.value.resize();
                }
              }, 100);
            });
          } else if (newTab === 'route-map-adjust-2') {
            nextTick(() => {
              setTimeout(() => {
                if (RouteMapAdjustTab2.value && RouteMapAdjustTab2.value.resize) {
                  RouteMapAdjustTab2.value.resize();
                }
              }, 100);
            });
          } else if (newTab === 'route-map-adjust-straight') {
            nextTick(() => {
              setTimeout(() => {
                if (RouteMapAdjustStraightTab.value && RouteMapAdjustStraightTab.value.resize) {
                  RouteMapAdjustStraightTab.value.resize();
                }
              }, 100);
            });
          } else if (newTab === 'route-schematic') {
            nextTick(() => {
              setTimeout(() => {
                if (RouteSchematicTab.value && RouteSchematicTab.value.resize) {
                  RouteSchematicTab.value.resize();
                }
              }, 100);
            });
      ...
      return {
        ...
        RouteAdjustGridScalingTab, // 網格縮放（路線調整）組件引用
        ...
        routeAdjustGridScalingContainerRef, // 網格縮放（路線調整）容器引用
        ...
        RouteMapAdjustTab, // 路線圖調整顯示元件引用
        routeMapAdjustContainerRef, // 路線圖調整容器引用
        RouteMapAdjustTab2, // 路線圖轉換骨架2顯示元件引用
        routeMapAdjust2ContainerRef, // 路線圖轉換骨架2容器引用
        RouteMapAdjustStraightTab, // 路線圖轉換直線骨架顯示元件引用
        routeMapAdjustStraightContainerRef, // 路線圖轉換直線骨架容器引用
        RouteSchematicTab, // 示意圖佈局骨架顯示元件引用
        routeSchematicContainerRef, // 示意圖佈局骨架容器引用
  ...
  <div class="d-flex flex-column h-100 overflow-hidden">
    ...
    <div class="d-flex justify-content-start my-bgcolor-gray-200 p-3">
      <div class="d-flex align-items-center rounded-pill shadow my-blur gap-1 p-2">
        ...
        <!-- 📄 選擇路線圖 GeoJSON 檢視按鈕 (Select Route Map GeoJSON Button) -->
        <button
          class="btn rounded-circle border-0 d-flex align-items-center justify-content-center my-btn-transparent my-font-size-xs"
          :class="{
            'my-btn-blue': activeUpperTab === 'select-route-map-geojson',
          }"
          :disabled="!isTabEnabled['select-route-map-geojson']"
          @click="$emit('update:activeUpperTab', 'select-route-map-geojson')"
          title="選擇路線圖 GeoJSON"
          style="width: 30px; height: 30px"
        >
          <i :class="getIcon('file_code').icon"></i>
        ...
        <button
          class="btn rounded-circle border-0 d-flex align-items-center justify-content-center my-btn-transparent my-font-size-xs"
          :class="{
            'my-btn-blue': activeUpperTab === 'route-schematic',
          }"
          :disabled="!isTabEnabled['route-schematic']"
          @click="$emit('update:activeUpperTab', 'route-schematic')"
          title="示意圖佈局（骨架顯示）"
          style="width: 30px; height: 30px"
        >
          <i :class="getIcon('project_diagram').icon"></i>
        </button>
        <!-- 🗺️ 路線圖調整按鈕 (Route Map Adjust Button) -->
        <button
          class="btn rounded-circle border-0 d-flex align-items-center justify-content-center my-btn-transparent my-font-size-xs"
          :class="{
            'my-btn-blue': activeUpperTab === 'route-map-adjust',
          }"
          :disabled="!isTabEnabled['route-map-adjust']"
          @click="$emit('update:activeUpperTab', 'route-map-adjust')"
          title="路線圖轉換骨架（從選擇路線圖載入）"
          style="width: 30px; height: 30px"
        >
          <i :class="getIcon('map_marked').icon"></i>
        </button>
        <!-- 🗺️ 路線圖轉換骨架2按鈕 -->
        <button
          class="btn rounded-circle border-0 d-flex align-items-center justify-content-center my-btn-transparent my-font-size-xs"
          :class="{
            'my-btn-blue': activeUpperTab === 'route-map-adjust-2',
          }"
          :disabled="!isTabEnabled['route-map-adjust-2']"
          @click="$emit('update:activeUpperTab', 'route-map-adjust-2')"
          title="路線圖轉換骨架2（從選擇路線圖載入）"
          style="width: 30px; height: 30px"
        >
          <i :class="getIcon('map_marked').icon"></i>
        </button>
        ...
        <button
          class="btn rounded-circle border-0 d-flex align-items-center justify-content-center my-btn-transparent my-font-size-xs"
          :class="{
            'my-btn-blue': activeUpperTab === 'route-map-adjust-straight',
          }"
          :disabled="!isTabEnabled['route-map-adjust-straight']"
          @click="$emit('update:activeUpperTab', 'route-map-adjust-straight')"
          title="路線圖轉換直線骨架（從選擇路線圖載入）"
          style="width: 30px; height: 30px"
        >
          <i :class="getIcon('map_marked').icon"></i>
        </button>
        ...
        <!-- 📊 網格縮放（路線調整）按鈕 (Route Adjust Grid Scaling Button) -->
        <button
          class="btn rounded-circle border-0 d-flex align-items-center justify-content-center my-btn-transparent my-font-size-xs"
          :class="{
            'my-btn-blue': activeUpperTab === 'route-adjust-grid-scaling',
          }"
          :disabled="!isTabEnabled['route-adjust-grid-scaling']"
          @click="$emit('update:activeUpperTab', 'route-adjust-grid-scaling')"
          title="網格縮放視覺化（路線調整）"
          style="width: 30px; height: 30px"
        >
          <i :class="getIcon('chart_line').icon"></i>
    ...
    <div class="flex-grow-1 overflow-auto">
      ...
      <!-- 網格縮放（路線調整）分頁內容 -->
      <div
        v-show="activeUpperTab === 'route-adjust-grid-scaling'"
        ref="routeAdjustGridScalingContainerRef"
        class="h-100"
      >
        <RouteAdjustGridScalingTab
          ref="RouteAdjustGridScalingTab"
          class="flex-grow-1 d-flex flex-column"
          :containerHeight="contentHeight"
          :isPanelDragging="isPanelDragging"
          :activeMarkers="activeMarkers"
      ...
      <!-- 📄 選擇路線圖 GeoJSON 檢視 -->
      <div v-show="activeUpperTab === 'select-route-map-geojson'" class="h-100">
        <SelectRouteMapGeojsonTab class="h-100" />
      </div>

      <!-- 🗺️ 路線圖調整（route_map_adjust）：從選擇路線圖載入之唯讀 Leaflet 顯示 -->
      <div
        v-show="activeUpperTab === 'route-map-adjust'"
        ref="routeMapAdjustContainerRef"
        class="h-100"
      >
        <RouteMapAdjustTab
          ref="RouteMapAdjustTab"
          class="flex-grow-1 d-flex flex-column"
          :containerHeight="contentHeight"
          :isActive="activeUpperTab === 'route-map-adjust'"
        />
      </div>

      <!-- 🗺️ 路線圖轉換骨架2（route_map_adjust_2）：從選擇路線圖載入之唯讀 Leaflet 顯示 -->
      <div
        v-show="activeUpperTab === 'route-map-adjust-2'"
        ref="routeMapAdjust2ContainerRef"
        class="h-100"
      >
        <RouteMapAdjustTab2
          ref="RouteMapAdjustTab2"
          class="flex-grow-1 d-flex flex-column"
          :containerHeight="contentHeight"
          :isActive="activeUpperTab === 'route-map-adjust-2'"
        />
      </div>

      <!-- 🗺️ 路線圖轉換直線骨架（route_map_adjust_straight）：從選擇路線圖載入之唯讀 Leaflet 顯示 -->
      <div
        v-show="activeUpperTab === 'route-map-adjust-straight'"
        ref="routeMapAdjustStraightContainerRef"
        class="h-100"
      >
        <RouteMapAdjustStraightTab
          ref="RouteMapAdjustStraightTab"
          class="flex-grow-1 d-flex flex-column"
          :containerHeight="contentHeight"
          :isActive="activeUpperTab === 'route-map-adjust-straight'"
        />
      </div>

      <!-- 🗺️ 示意圖佈局（schematic_rma_*）：載入之骨架獨立顯示（顏色同骨架） -->
      <div
        v-show="activeUpperTab === 'route-schematic'"
        ref="routeSchematicContainerRef"
        class="h-100"
      >
        <RouteSchematicTab
          ref="RouteSchematicTab"
          class="flex-grow-1 d-flex flex-column"
          :containerHeight="contentHeight"
          :isActive="activeUpperTab === 'route-schematic'"
        />
      </div>
