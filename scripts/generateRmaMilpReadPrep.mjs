...
/**
 * 離線預算全球城市：路線圖轉換骨架2 → ⑨ 正規化 → 站點與路線調整前置（從⑨匯入）結果。
 *
 * 執行：node --no-warnings --loader ./loader.mjs scripts/generateRmaMilpReadPrep.mjs
 * 選項：
 *   --city=taiwan-taipei     只跑單一城市
 *   --skip-existing          已有 JSON 則跳過
 * 產物：public/data/metro/rma_milp_read_prep/{cityId}.json
 *        public/data/metro/rma_milp_read_prep/_index.json
 */
import { readFileSync, writeFileSync, mkdirSync, existsSync } from 'node:fs';
import { join } from 'node:path';
import { buildRmaMilpReadPrepFromMetroGeojson } from '../src/utils/routeMapAdjust/pipeline/rmaPrepFromMetro.js';

const ROOT = new URL('..', import.meta.url).pathname;
const CATALOG_PATH = join(ROOT, 'public/data/metro/_catalog.json');
const OUT_DIR = join(ROOT, 'public/data/metro/rma_milp_read_prep');
...
console.log(`預算 ${cities.length} 個城市 → ${OUT_DIR}\n`);
...
for (let i = 0; i < cities.length; i++) {
  ...
  try {
    ...
    const result = buildRmaMilpReadPrepFromMetroGeojson(fc, {
      cityId: city.id,
      city: city.city,
      country: city.country,
      cityZh: city.cityZh,
    ...
    console.log(
      `  ✓ ${tag}: ${result.payload.segmentCount} 段（骨架 ${result.payload.skeletonEdgeCount} 邊）`
    );
  } catch (e) {
    fail++;
