...
1. **步長**：單次 |Δx| ≤ 路網寬 W 的 1/10；|Δy| ≤ 路網高 H 的 1/10（見 `moveLimits`）
2. **方位**（45°/135° 分隔）：|dx|≥|dy| 偏水平 ↔ |dy|>|dx| 偏垂直，不可互換
3. 非相鄰 connect 邊不可在內部交叉
4. 共線 connect 邊不可重疊
5. 不同 connect 節點不可佔同一格
6. connect 端點不可壓在無關路線上
